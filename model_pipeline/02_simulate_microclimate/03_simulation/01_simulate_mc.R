...
# Rscript 01_simulate_mc.R --config config.toml --year <y> --timestep <t> [--verbose]

# Example config.toml
#
# Microclimate Simulation Configuration File
#
#     [simulation]
#     # Grid dimensions
#     x_dim = 50
#     y_dim = 50
#
#     # Number of temperature metrics to calculate
#     n_temp_metrics = 14
#
#     [processing]
...
#     chunk_size = 50
#
#     # Number of CPU cores to use for parallel processing within this job
#     # Set to "auto" for auto-detection, or specify a number
#     cores = "auto"
#
#     [paths]
#     # Region identifier
#     region = "regua" #  "regua" or "pirineus"
#     replicate = 2 # Replicate forest number, one of {0, 1, 2}
#
#     # Base directory containing microhabitat matrix (usually "../modve_data/modve_output")
#     veg_indir = "/path/to/<region>/microhabitat/rep<rep>/microhabitatMatrix<ts>.rds"
#
#     # Path to climdata csv file (including all years of the scenario)
#     # - e.g. "../modve_data/mc_input/regua/scenarios/climdata_era5_cmip6_1981-2100_ssp245_v1.csv"
#     clim_path = "/path/to/climate/data.csv"
#
#     # Base directory containing micropoint input data
#     in_dir = "/path/to/<region>" # Usually "../modve_data/mc_input", includes soil and vegetation parameter files
#
#     # Base directory for microclimate output
#     outdir = "/path/to/<region>/<rep>/<year>/mc_x<x>_y<y>.rds" # Usually "../modve_data/mc_output"
#
#     [output]
#     # Enable verbose output (can be overridden by --verbose command line flag)
#     verbose = false
...
# Get indices of cells within the chunk of cells to process in parallel
...
# Generate list of x, y indices for which simulations are needed
...
# Aggregate microclimate outputs from hourly to annual
...
process_cell <- function(x, y, year, n_temp_metrics = 14, microhab_path,
  ...
  # Extrat lat/lon in EPSG:4326 from cell indices and given raster
  coords <- indices2coords(x, y, terra::unwrap(vegp_reg$pai))[c("x", "y")]
  lon <- coords[[1]]
  lat <- coords[[2]]

  # Extract parameters for that coordinate
  ...
  # Result list for array with date/time, and (X x Y x Z)-matrix for each MC variable
  res <- list()

  for (i in seq_along(heights)) { # Simulate mc for each height at (x, y)
    h <- heights[i]
    if (verbose) {
    ...
    for (var in exp_vars) {
      if (i == 1) { # For the first iteration initialize array for each variable
        res[[var]] <- array(NA, dim = c(length(heights), length(mout[[var]])))
      }
      # Populate array with model output
      res[[var]][i, ] <- mout[[var]]
    }
  }

  # Aggregate from hourly to yearly microclimate data
...
option_list <- list(
  make_option(c("-c", "--config"), type = "character",
              help = "Path to TOML configuration file (no default)", metavar = "FILE"),
  make_option(c("-k", "--chunk"), type = "integer",
              help = "Number of chunk that determines which cells to simulate [default %default]"),
  make_option(c("-y", "--year"), type = "integer",
              help = "Year for which microclimate will be simulated (must be within climdata range) (no default)"),
  make_option(c("-t", "--timestep"), type = "integer",
              help = "Time step to take from forest simulation (no default)"),
...
# --- Extract configuration values with defaults and validation

# Simulation parameters
x_dim <- if (!is.null(config$simulation$x_dim)) config$simulation$x_dim else 50
y_dim <- if (!is.null(config$simulation$y_dim)) config$simulation$y_dim else 50
n_temp_metrics <- if (!is.null(config$simulation$n_temp_metrics)) config$simulation$n_temp_metrics else 14
...
paths_to_check <- list(
  veg_indir = veg_indir_base,
  indir = indir_base,
  climdata = climdata_path
)

for (path_name in names(paths_to_check)) {
  path <- paths_to_check[[path_name]]
  if (!file.exists(path)) {
    stop(paste("Error: The path for", path_name, "does not exist:", path))
  }
}
...
if (verbose) {
  ...
  immediateMessage("  Main input dir:", indir, "\n")
...
# ----------------------------
# Process chunk of cells in parallel
# ----------------------------
...
output <- foreach(pair_idx = seq(first_cells_idx, last_cells_idx),
                 .export = c("process_cell", "mean_diurnal_temp_vectorized", "month_stats_vectorized",
                            "hourly_to_daily_medians", "aggregate_mc", "immediateMessage",
                             "extract_params", "indices2coords")) %dorng% {
  x <- cells$x[pair_idx]
  y <- cells$y[pair_idx]
