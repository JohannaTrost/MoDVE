...
access_token <- "fcaf31a3-dd2c-46e5-849b-545aa5a6c5ae"
...
climate_dir <- file.path(dirout, "mc_input", "climate")
if (!dir.exists(climate_dir)) {
  dir.create(climate_dir, recursive = TRUE)
  cat("Created directory:", climate_dir, "\n")
}

# Define output path
pathout <- file.path(climate_dir, "era5")
if (!dir.exists(pathout)) {
  dir.create(pathout, recursive = TRUE)
  cat("Created directory:", pathout, "\n")
}
...
for (i in seq_along(years)) {
  ...
  for (month in seq(1, 12)) {
    ...
    nc_path <- file.path(pathout, req[[month]]$target)
