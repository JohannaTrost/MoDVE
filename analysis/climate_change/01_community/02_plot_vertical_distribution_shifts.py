# -----
# Visualize vertical species distribution and shifts due to climate change
    ...
    for forest in forests:
        for sp in species_pools:
            ...
            for ts in range(ts_start, ts_end + 1):
                path = (base_dir / scenario / "communities" / f"forest{forest}" /
...
year = 2030
df_plot = species_distr[species_distr["Year"] == year]
...
plt.tight_layout()
plt.savefig(DirectoryPlots / f"cc_vs_no_cc_species_range_box_{year}.png")
...
plt.tight_layout()
plt.savefig(DirectoryPlots / f"cc_vs_no_cc_species_range_violin_{year}.png")
