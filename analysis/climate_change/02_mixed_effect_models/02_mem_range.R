# Fit mixed-effects model (MEM) for species range (max-min)
...
DirectoryPlots <- file.path("../modve_figs/climate_change/mem/range")
...
pdf(file.path(DirectoryPlots, "diag_range_res_qq_dharma_dispformula.pdf"),
    width = 10, height = 5)
plot(simulationOutput)
dev.off()
...
pdf(file.path(DirectoryPlots, "range_res_grouped_dispformula.pdf"), width = 7, height = 7)
print(p2 / p1)
dev.off()
...
# Plot histograms
pdf(file.path(DirectoryPlots, "diag_rf_sp_normal_glmmtmb_dispformula.pdf"))
hist(species_pool)
dev.off()

pdf(file.path(DirectoryPlots, "diag_rf_sp_cc_normal_glmmtmb_dispformula.pdf"))
hist(species_pool_scenario)
dev.off()

pdf(file.path(DirectoryPlots, "diag_rf_forest_normal_glmmtmb_dispformula.pdf"))
hist(forest_id)
dev.off()
