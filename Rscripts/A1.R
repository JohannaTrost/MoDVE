...
if (MicrohabitatType == 1 || MicrohabitatType == 2) {
  ...
  GlobalForest <- read.table(path_to_forest_global,
                             sep = "\t",
                             row.names = 1)
...
if (MicrohabitatType == 1 || MicrohabitatType == 2) {
  file.copy(
    file.path(DirectoryGroIMP, model_dir_name, forest_global_param_name),
    DirectoryMatrices,
    overwrite = FALSE
  )
  file.copy(
    file.path(DirectoryGroIMP, model_dir_name, forest_pass_param_name),
    DirectoryMatrices,
    overwrite = FALSE
  )
}
  ...
  for (i in timeStepStart:timeStepEnd) {
    ...
    if (i != timeStepStart) {
      ...
      ShootsBegin <- read.table(
        file.path(src_dir, shootFileNameOld),
        sep = "\t",
        header = TRUE,
        skip = 1
      )
      TrunksBegin <- read.table(
        file.path(src_dir, trunkFileNameOld),
        sep = "\t",
        header = TRUE,
        skip = 8
      )
    ...
    ShootsEnd <- read.table(
      file.path(src_dir, shootFileNameNew),
      sep = "\t",
      header = TRUE,
      skip = 1
    )
    TrunksEnd <- read.table(
      file.path(src_dir, trunkFileNameNew),
      sep = "\t",
      header = TRUE,
      skip = 8
    )
        ...
        for (y in seq_len(numY)) {
          ...
          for (z in seq_len(numZ)) {
            ...
            if (TotalSurfaceAreaOpt == 1) {
              ...
              voxel_branch_surface <- (ShootsBegin$length[j] / (numX * numY * numZ)) *
                ShootsBegin$diameter[j] * pi / 2

              Mat_surface_per_cell[xid, yid, zid] <- Mat_surface_per_cell[xid, yid, zid] +
                voxel_branch_surface
            }
            ...
            if (SurfaceAreaLossOpt == 1 & TotalDead > 0) {
              if (j == locDeadSegments[CounterDead]) {
                ...
                lost_surface <- (ShootsBegin$length[j] / (numX * numY * numZ)) *
                  ShootsBegin$diameter[j] * pi / 2
            ...
            if (AverageWeightedAngles == 1) {
              ...
              position1 <- c(ShootsBegin$xbegin[j],
                             ShootsBegin$ybegin[j],
                             ShootsBegin$zbegin[j])
              position2 <- c(ShootsBegin$xend[j],
                             ShootsBegin$yend[j],
                             ShootsBegin$zend[j])
              ...
              tmp1 <- (Mat_surface_per_cell[xid, yid, zid] - (
                ShootsBegin$length[j] / (numX * numY * numZ) *
                  ShootsBegin$diameter[j] * pi / 2
                )) / Mat_surface_per_cell[xid, yid, zid] *
                Mat_weighted_angle_per_cell[xid, yid, zid]

              tmp2 <- ShootsBegin$length[j] / (numX * numY * numZ) *
                   ShootsBegin$diameter[j] * pi / 2 /
                Mat_surface_per_cell[xid, yid, zid] * ShootAngle
    ...
    if (LightConditionsOpt == 1) {
      ...
      colnames <- as.character(read.table(
        file.path(src_dir, voxelsFileName),
        sep = "\t",
        skip = 1,
        nrows = 1
      ))
    ...
    Microhabitat <- array(
      rep(0, dimPlot[1] * dimPlot[2] * dimPlot[3] * MatrixDimension),
